...
var vm = function () {
    ...
    self.displayName = 'Participant Countries List';
    ...
    self.addFavorite = function (data, event) {
        if (!data) return;
        let favoriteCountries = JSON.parse(window.localStorage.getItem('favoriteCountries') ?? '[]');

        if (!favoriteCountries.includes(data.Id)) favoriteCountries.push(data.Id);
        else favoriteCountries.splice(favoriteCountries.indexOf(data.Id), 1);

        let icon = event.target.closest('.card-body').querySelector('i');
        icon.classList.toggle('fa-heart-o');
        icon.classList.toggle('fa-heart');

        window.localStorage.setItem('favoriteCountries', JSON.stringify(favoriteCountries));
    }
    ...
    self.filterFavorites = function (data, event) {
        ...
        if (!self.favoritesFilter) {
            getDataPaginated(self.baseUri() + "?page=1&pageSize=500", r => r.favorite);
        } else {
    ...
    function getDataPaginated (composedUri, filter = null) {
        ajaxHelper(composedUri, 'GET').done(function (data) {
            ...
            data.Records.forEach(record => {
                record.favorite = favoriteCountries.includes(record.Id);
            })

...
var vm = function () {
    ...
    self.addFavorite = function (data, event) {
        if (!data) return;
        let favoriteAthletes = JSON.parse(window.localStorage.getItem('favoriteAthletes') ?? '[]');

        if (!favoriteAthletes.includes(data.Id)) favoriteAthletes.push(data.Id);
        else favoriteAthletes.splice(favoriteAthletes.indexOf(data.Id), 1);

        let icon = event.target.closest('.card-body').querySelector('i');
        icon.classList.toggle('fa-heart-o');
        icon.classList.toggle('fa-heart');

        window.localStorage.setItem('favoriteAthletes', JSON.stringify(favoriteAthletes));
    }
    ...
    self.filterFavorites = function (data, event) {
        ...
        if (!self.favoritesFilter) {
            getDataPaginated(self.baseUri() + "?page=1&pageSize=200000", r => r.favorite);
        } else {
    ...
    function getDataPaginated (composedUri, filter = null) {
        ajaxHelper(composedUri, 'GET').done(function (data) {
            ...
            data.Records.forEach(record => {
                record.favorite = favoriteAthletes.includes(record.Id);
            })

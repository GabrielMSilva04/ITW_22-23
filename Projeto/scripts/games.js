...
var vm = function () {
    ...
    self.addFavorite = function (data, event) {
        if (!data) return;
        let favoriteGames = JSON.parse(window.localStorage.getItem('favoriteGames') ?? '[]');

        if (!favoriteGames.includes(data.Id)) favoriteGames.push(data.Id);
        else favoriteGames.splice(favoriteGames.indexOf(data.Id), 1);
    ...
    function getDataPaginated (composedUri, filter = null) {
        ajaxHelper(composedUri, 'GET').done(function (data) {
            ...
            data.Records.forEach(record => {
                record.favorite = favoriteGames.includes(record.Id);
            })

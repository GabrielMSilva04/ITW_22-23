...
var vm = function () {
    ...
    self.addFavorite = function (data, event) {
        if (!data) return;
        let favoriteCompetitions = JSON.parse(window.localStorage.getItem('favoriteCompetitions') ?? '[]');

        if (!favoriteCompetitions.includes(data.Id)) favoriteCompetitions.push(data.Id);
        else favoriteCompetitions.splice(favoriteCompetitions.indexOf(data.Id), 1);

        let icon = event.target.closest('.card-body').querySelector('i');
        icon.classList.toggle('fa-heart-o');
        icon.classList.toggle('fa-heart');

        window.localStorage.setItem('favoriteCompetitions', JSON.stringify(favoriteCompetitions));
    }
    ...
    self.filterFavorites = function (data, event) {
        ...
        if (!self.favoritesFilter) {
            getDataPaginated(self.baseUri() + "?page=1&pageSize=1000", r => r.favorite);
        } else {
    ...
    function getDataPaginated (composedUri, filter = null) {
        ajaxHelper(composedUri, 'GET').done(function (data) {
            ...
            data.Records.forEach(record => {
                record.favorite = favoriteCompetitions.includes(record.Id);
            })
